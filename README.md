...
| 多账号 + 配额故障转移 | 可登录多个 Google 账号；按 `accountStrategy` 轮询或优先默认账号；某账号被 429 时按两档冷却：provider 明说重置时间的停到那一刻（多长都照实停），这次调用不再换号、直接把报错抛出；没说重置时间的停 5 分钟并换下一个号 |
...
| 配额冷却 | 黄点 + 配额冷却中，约 N 分钟后恢复 + **清除冷却**按钮 | 冷却只是池子对 provider 等待的读数，觉得读错了就按这个按钮清掉它。provider 明说重置时间的（硬冷却）停到那一刻，N 就是那段真实时间，期间绝不被交出；响应体没说、或令牌刷新失败的（软冷却）是猜的窗口，封顶 5 分钟，期间仍可能被尝试，只是排在健康账号之后 |
...
账号清单是插件自有的一份注册表 `~/.dsh/antigravity-accounts.json`（0600）：每个账号一条，记 id（邮箱的哈希）、邮箱、项目、加入时间、最后使用时间，以及被配额停用的截止时刻与档位（provider 明说的重置时间是硬冷却，猜测的窗口是软冷却）；凭据本身也在里面。
...
故障转移发生在**一次调用内部**：某个账号被 429 或 401/403（凭据）拒绝时，适配器把失败报给账号池，池按下面两档给它打上冷却，同一次调用立刻换下一个账号重发；全部账号都不行时抛出的仍是**最后那个真实错误**，而不是笼统的「没有凭据」。传输类失败（5xx / 网络）不换号 —— 换谁都一样，只会把一次坏请求乘上账号数。

冷却分两档，由账号条目里的 `cooldownKind` 记录。**硬冷却**：429 响应体带着 `Resets in 1h31m29s` 这类重置时间，那是 provider 量化过的事实，账号停到那一刻，**多长都照实停**（63 小时就停 63 小时），期间绝不被交出，也不参与任何重试；带重置时间的 429 因此是终局 —— 适配器不再换号，直接把 provider 的真实报错抛给调用方，因为再交一个账号出去只会白吃一个 429，而注册表早就知道答案。**软冷却**：429 但响应体没说重置时间，或令牌刷新失败（停 5 分钟）—— 这是猜的窗口，仍封顶 5 分钟，期间账号可以被尝试（猜错一次的成本是一个请求），只是优先级排在健康账号之后。

选号顺序就是这三档：健康账号 → 软冷却账号 → 都没有就返回 `undefined`；**全部账号都在硬冷却时同样返回 `undefined`**（快速失败），不兜底交出最早恢复的那个。注册表里缺 `cooldownKind` 的旧条目按硬冷却读 —— 能超过 5 分钟的冷却只可能来自 provider 明说的重置时间，猜的那条路一直封在 5 分钟。

所有 429 在错误对象上都带 `quotaScoped: true`，池子据此软冷却并换号；但只有报文确实是配额语义时 `code` 才是 `QUOTA_EXCEEDED`，否则保持 `RATE_LIMIT` —— DSH 的默认重试策略重试 `RATE_LIMIT`、不重试 `QUOTA_EXCEEDED`，把一次突发限流编成后者等于让它永久退出自动重试，文案也不把限流说成「配额已用尽」。

换号之前，同一次尝试还会走完**剩余的端点**：配额池是按端点独立的（2026-09-18 现场实测：同一秒里 `daily` 端点返回 200，而 `cloudcode-pa` 返回 429），所以 429 不能终止端点遍历——那会把一个还能用的端点白白丢掉。鉴权失败（401/403）是例外，被拒的凭据在每个端点上都会被拒，所以立刻收手，不为同一个坏 token 打三个端点。

每次端点尝试还有一个**首包熔断**：120 秒内拿不到响应头就放弃这个端点、换下一个。计时器在响应头到达的瞬间清除，所以它只挡「连上了却永远不回答」的挂死，不会掐断一段正常生成的长流（现场排队最严重时首包 12–39 秒，离这个阈值两个数量级）。
...
实时记录与日志副本是**同一次调用的两个视图**，而它们的键天生不会撞（实时用随机
uuid、扫描用 `会话:事件序号`）：只看主键的话，每一次被观察到的调用都会再被写一遍，
面板上的调用数、令牌与费用全部翻倍。所以扫描还要按「哪一次调用」判重——同一会话、
同一 input/output 令牌数、时间相差不超过 60 秒（实测同一次调用两边只差 35–60 ms）。
命中的日志副本不再导入；**已经写进库的**副本由同一条规则在每次扫描开始时删除，
所以老库会在下次打开面板时自动从"双倍"回到真实值。插件装上之前的历史没有实时孪生，
原样保留。
