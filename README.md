...
| **全部移除** | 行右上角点 **移除**，或对最后一个账号点 **退出登录** | 清空注册表与全部镜像，撤掉 `account` 标记，行随之消失，回到下拉里 |
...
| `GOOGLE_ANTIGRAVITY_DATA` / `GOOGLE_ANTIGRAVITY_TOKEN` | 环境变量注入，优先级最高（在账号池里表现为一个不落盘、不冷却的 `env` 账号） |

> 登录成功后会写注册表，并把默认账号同时写进 seam 记录与镜像文件；移除账号会同步三者。任何路径都不会触碰 `~/.omp`。
...
pnpm test                  # 构建后跑全部单元测试（124 宿主/浏览器 + 59 用量），无网络、无真实凭据
